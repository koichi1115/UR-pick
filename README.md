...
- **ユーザープロファイル管理**: スワイプ履歴と好みの自動学習
- **セキュアな実装**: Helmet、Rate Limiting、入力サニタイゼーション
- **包括的なテスト**: バックエンド31テスト、フロントエンド8テスト（全て成功）
...
- `POST /api/recommendations` - 商品レコメンデーションを取得
  - Rate Limit: 20リクエスト/15分

### User Management
- `POST /api/users` - 新規ユーザー作成
- `GET /api/users/:userId` - ユーザー情報取得
- `PUT /api/users/:userId/preferences` - ユーザー設定更新
- `POST /api/users/:userId/swipes` - スワイプアクション記録
- `GET /api/users/:userId/swipes` - スワイプ履歴取得
  - Rate Limit: 100リクエスト/15分（全API共通）
...
- **Helmet**: セキュリティヘッダーの自動設定（CSP、HSTS等）
- **Rate Limiting**:
  - 一般API: 100リクエスト/15分
  - レコメンデーションAPI: 20リクエスト/15分
- **入力サニタイゼーション**: XSS攻撃防止
- **リクエストバリデーション**: express-validatorによる厳密な検証
- **リクエストタイムアウト**: 30秒の自動タイムアウト
